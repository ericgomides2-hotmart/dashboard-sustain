...
Uso:
  python3 extract-jira-data.py

Pré-requisitos:
  - pip install requests
  - Variáveis de ambiente:
    JIRA_BASE_URL  (ex: https://hotmart.atlassian.net)
...
# Custom field IDs — mapeados via JIRA REST API
...
def extract_option(val):
    """Extrai valor de um campo select/cascading."""
    if val is None:
        return ""
    if isinstance(val, dict):
        if "child" in val and val["child"]:
            return val["child"].get("value", "")
        return val.get("value", val.get("name", ""))
    return str(val)


def extract_partner(val):
    """Extrai valor do campo Partner (pode ser array de options)."""
    if val is None:
        return ""
...
def fetch_all_issues():
    """Busca todos os tickets via JIRA REST API v3 com paginação por token."""
    ...
    while True:
        ...
        for issue in data.get("issues", []):
            ...
            ticket = {
                ...
                "camadaServico": extract_option(cs.get("child")) if isinstance(cs, dict) and cs.get("child") else extract_option(cs),
                "priorityCX": extract_option(f.get(CF_PRIORITY_CX)),
                "segmentacao": extract_option(f.get(CF_SEGMENTACAO_CX)),
